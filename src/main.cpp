/*
    Copyright (C) 2017-2019 Alexey Kozlov, Alexandros Stamatakis, Diego Darriba,
   Tomas Flouri
...
enum class RaxmlRunPhase { start, mlsearch, bootstrap, finish };
...
struct RaxmlInstance {
  ...
  /* IDs of the trees that have been already inferred (eg after resuming from a
   * checkpoint) */
  IDSet done_ml_trees;
  IDSet done_bs_trees;
  ...
  /* this is just a dummy random tree used for convenience, e,g, if we need tip
   * labels or just 'any' valid tree for the alignment at hand */
  Tree random_tree;
  ...
  vector<vector<doubleVector>>
      persite_loglh; // per-tree -> per-partition -> per-site

  vector<RaxmlWorker> workers;
  RaxmlWorker &get_worker() {
    return workers.at(ParallelContext::local_group_id());
  }

  RaxmlInstance()
      : bs_converged(false), run_phase(RaxmlRunPhase::start), used_wh(0) {}
};
...
struct RaxmlWorker {
  ...
  size_t total_num_searches() const {
    return start_trees.size() + bs_trees.size();
  }
};

void print_banner() {
  LOG_INFO << endl
           << "RAxML-NG v. " << RAXML_VERSION << " released on " << RAXML_DATE
           << " by The Exelixis Lab." << endl;
  LOG_INFO << "Developed by: Alexey M. Kozlov and Alexandros Stamatakis."
           << endl;
  LOG_INFO
      << "Contributors: Diego Darriba, Tomas Flouri, Benoit Morel, "
         "Sarah Lutteropp, Ben Bettisworth, Julia Haag, Anastasis Togkousidis."
      << endl;
  ...
  if (sysutil_task_cpu_cores(true) < sysutil_get_cpu_cores())
    LOG_INFO << " (allocated: " << sysutil_task_cpu_cores(true)
             << " physical cores)";
  LOG_INFO << endl << endl;
}
...
void init_part_info(RaxmlInstance &instance) {
  ...
  if (opts.msa_format == FileFormat::binary ||
      (opts.msa_format == FileFormat::autodetect &&
       RBAStream::rba_file(opts.msa_file))) {
    ...
    if (opts.command == Command::sitelh) {
      throw runtime_error("Alignments in RBA format are not supported in "
                          ...
                          "PHYLIP/FASTA instead.");
    }

    if (!opts.model_file.empty()) {
      LOG_WARN
          << "WARNING: The model you specified on the command line ("
          << opts.model_file << ") will be ignored " << endl
          << "         since the binary MSA file already contains a model "
             "definition."
          << endl
          << "         If you want to change the model, please re-run RAxML-NG "
          << endl
          << "         with the original PHYLIP/FASTA alignment and --redo "
             "option."
          << endl
          << endl;
    }
    ...
    if (!opts.weights_file.empty()) {
      LOG_WARN << "WARNING: Alignment site weights file (" << opts.weights_file
               << ") will be ignored!" << endl
               << "NOTE:    Custom site weights are not allowed in combination "
                  "with RBA input."
               << endl
               << endl;
    }

    LOG_INFO_TS << "Loading binary alignment from file: " << opts.msa_file
                << endl;

    auto rba_elem = opts.use_rba_partload ? RBAStream::RBAElement::metadata
                                          : RBAStream::RBAElement::all;
    ...
    LOG_INFO_TS << "Alignment comprises " << parted_msa.taxon_count()
                << " taxa, " << parted_msa.part_count() << " partitions and "
  ...
  /* check if model is a file */
    ...
    throw runtime_error(
        "Please specify an evolutionary model with --model switch");

  assert(parted_msa.part_count() > 0);

  /* make sure that linked branch length mode is set for unpartitioned
   * alignments */
  if (parted_msa.part_count() == 1) {
    opts.brlen_linkage = PLLMOD_COMMON_BRLEN_LINKED;
    if (opts.safety_checks.isset(SafetyCheck::model) &&
        parted_msa.model(0).param_mode(PLLMOD_OPT_PARAM_BRANCH_LEN_SCALER) !=
            ParamValue::undefined)
      throw runtime_error("Branch length scalers (+B) are not supported for "
  ...
  if (opts.brlen_linkage == PLLMOD_COMMON_BRLEN_SCALED) {
    for (auto &pinfo : parted_msa.part_list())
      pinfo.model().set_param_mode_default(PLLMOD_OPT_PARAM_BRANCH_LEN_SCALER,
                                           ParamValue::ML);
  }
  ...
  if (parted_msa.part_count() > 1 && freerate_count > 0 &&
      opts.brlen_linkage == PLLMOD_COMMON_BRLEN_LINKED) {
    throw runtime_error("LG4X and FreeRate models are not supported in linked "
                        "branch length mode.\n"
                        "Please use the '--brlen scaled' option to switch into "
                        "proportional branch length mode.");
  }
}

void print_reduced_msa(const RaxmlInstance &instance,
                       const PartitionedMSAView &reduced_msa_view) {
  ...
  LOG_INFO << "\nNOTE: Reduced alignment (with duplicates and gap-only "
              "sites/taxa removed) "
              "\nNOTE: was saved to: ";
...
bool check_msa_global(const MSA &msa, const Options &opts) {
  ...
  if (msa.size() < 4) {
    if (msa.size() == 3 && opts.command == Command::evaluate) {
      ...
      LOG_WARN << "\nWARNING: Your alignment contains less than 4 sequences! "
               << endl;
    } else {
  ...
  pllmod_msa_stats_t *stats =
      pllmod_msa_compute_stats(msa.pll_msa(), 4,
  ...
  if (stats->dup_taxa_pairs_count > 0) {
    ...
    for (unsigned long c = 0; c < stats->dup_taxa_pairs_count; ++c) {
      ...
      LOG_ERROR << "ERROR: Sequences " << id1 + 1 << " and " << id2 + 1
                << " have identical name: " << msa.label(id1) << endl;
    }
...
bool check_msa(RaxmlInstance &instance) {
  ...
  if (opts.safety_checks.isset(SafetyCheck::msa_names)) {
    ...
    for (const auto &taxon : parted_msa.taxon_names()) {
      if (taxon.find_first_of(invalid_chars) != std::string::npos) {
        ...
        while ((i = fixed_name.find_first_of(invalid_chars, i)) !=
               std::string::npos)
          fixed_name[i++] = '_';
  ...
  if (opts.safety_checks.isset(SafetyCheck::msa_dups)) {
    ...
    pllmod_msa_stats_t *stats =
        pllmod_msa_compute_stats(pll_msa, 4,
                                 pll_map_nt, // map is not used here
                                 NULL, stats_mask);
  ...
  for (auto &pinfo : parted_msa.part_list()) {
    ...
    pllmod_msa_errors_t *errs =
        pllmod_msa_check(pinfo.msa().pll_msa(), pinfo.model().charmap());
    ...
    if (errs) {
      if (errs->invalid_char_count > 0) {
        ...
        for (unsigned long c = 0; c < errs->invalid_char_count; ++c) {
          auto global_pos =
              parted_msa.full_msa_site(part_num, errs->invalid_char_pos[c]);
          LOG_ERROR << "ERROR: Invalid character in sequence "
                    << errs->invalid_char_seq[c] + 1 << " at position "
                    << global_pos + 1 << ": " << errs->invalid_chars[c] << endl;
        }
        part_num++;
        continue;
      }
    ...
    if (opts.safety_checks.isset(SafetyCheck::msa_allgaps)) {
      ...
      if (stats->gap_cols_count > 0) {
        ...
        if (gap_cols.size() < pinfo.msa().length() &&
          ...
          /* Special case: partition consists of all-gap columns only (no
           * informative sites) -> remove all-gap partition from the .reduced
           * file, BUT keep it for the current run. NOTE: all gap columns will
           * be compressed into a single pattern -> no performance loss */
          parted_msa_view.exclude_sites(part_num, gap_cols);
        }
      }

      std::set<size_t> cur_gap_seq(stats->gap_seqs,
  ...
  if (total_gap_cols > 0) {
    LOG_WARN << "\nWARNING: Fully undetermined columns found: "
             << total_gap_cols << endl;
    msa_corrected = true;
  }
  ...
  if (!gap_seqs.empty()) {
    ...
    for (auto c : gap_seqs) {
      ...
      LOG_VERB << "WARNING: Sequence #" << c + 1 << " ("
               << parted_msa.taxon_names().at(c) << ") contains only gaps!"
               << endl;
    }
    LOG_WARN << "WARNING: Fully undetermined sequences found: "
             << gap_seqs.size() << endl;
  }
  ...
  if (!dup_seqs.empty()) {
    ...
    for (const auto &p : dup_seqs) {
      ...
      LOG_WARN << "WARNING: Sequences " << parted_msa.taxon_names().at(p.first)
               << " and " << parted_msa.taxon_names().at(p.second)
  ...
  if (!instance.opts.nofiles_mode &&
      (msa_corrected || !parted_msa_view.identity())) {
  ...
  if (!parted_msa_view.taxon_name_map().empty()) {
    ...
    for (auto &it : parted_msa_view.taxon_name_map())
      LOG_ERROR << "ERROR: Following taxon name contains invalid characters: "
                << it.first << endl;

    LOG_ERROR << endl;
    LOG_INFO << "NOTE: Following symbols are not allowed in taxa names to "
                "ensure Newick compatibility:\n"
                "NOTE: \" \" (space), \";\" (semicolon), \":\" (colon), \",\" "
                "(comma), "
                "\"()\" (parentheses), \"'\" (quote). "
             << endl;
    LOG_INFO << "NOTE: Please either correct the names manually, or use the "
                "reduced alignment file\n"
...
void check_models(const RaxmlInstance &instance) {
  ...
  for (const auto &pinfo : instance.parted_msa->part_list()) {
    ...
    const auto &freqs = freq_mode == ParamValue::empirical
                            ? stats.emp_base_freqs
                            : model.base_freqs(0);
    ...
    if (opts.safety_checks.isset(SafetyCheck::model_lg4_freqs)) {
      if ((model.name() == "LG4X" || model.name() == "LG4M") &&
        ...
        throw runtime_error(
            "Partition \"" + pinfo.name() +
            "\": You specified LG4M or LG4X model with shared stationary based "
            "frequencies (" +
            model.to_string(false) +
            ").\n"
            "Please be warned, that this is against the idea of LG4 models and "
            "hence it's not recommended!" +
            "\n"
            "If you know what you're doing, you can add --force command line "
            "switch to disable this safety check.");
      }
    }
        ...
        for (unsigned int i = 0; i < freqs.size(); ++i) {
          if (freqs[i] < PLL_EIGEN_MINFREQ) {
            ...
            LOG_WARN << "WARNING: State " << to_string(i)
                     << (instance.parted_msa->part_count() > 1
                             ? " in partition " + pinfo.name()
                             : "")
                     << " has very low frequency (" << FMT_PREC9(freqs[i])
                     << ")!" << endl;
    ...
    if (model.num_submodels() > 1 &&
      ...
      throw runtime_error("Invalid model " + model.to_string(false) +
                          " in partition " + pinfo.name() +
                          ":\n"
                          "Mixture models with ML estimates of "
                          "rates/frequencies are not supported yet!");
    }

    // check partitions which contain invariant sites and have ascertainment
    // bias enabled
    if (opts.safety_checks.isset(SafetyCheck::model_asc_bias)) {
      if (model.ascbias_type() != AscBiasCorrection::none &&
          stats.inv_count() > 0) {
        throw runtime_error(
            "You enabled ascertainment bias correction for partition " +
            pinfo.name() + ", but it contains " + to_string(stats.inv_count()) +
            " invariant sites.\n"
            "This is not allowed! Please either remove invariant sites from "
            "MSA "
  ...
  if (opts.safety_checks.isset(SafetyCheck::model_overfit)) {
    if (instance.parted_msa->part_count() > 1) {
      ...
      string errmsg =
          "Number of free parameters (K=" + to_string(free_params) +
          ") is larger than alignment size (n=" + to_string(sample_size) +
          ").\n" +
          "       This might lead to overfitting and compromise tree inference "
          "results!\n" +
          "       Please consider revising your partitioning scheme, "
          "conducting formal model selection\n" +
          "       and/or using linked/scaled branch lengths across "
          "partitions.\n" +
...
void check_tree(const PartitionedMSA &msa, const Tree &tree,
  ...
  if (require_binary && !tree.binary()) {
    throw runtime_error(
        "Binary tree expected, but a tree with multifurcations provided!");
  }
  ...
  unordered_set<string> msa_labels(msa.taxon_names().cbegin(),
                                   msa.taxon_names().cend());

  for (const auto &tip : tree.tip_labels()) {
    if (!tree_labels.insert(tip.second).second) {
      LOG_ERROR << "ERROR: Taxon name appears more than once in the tree: "
                << tip.second << endl;
      duplicate_taxa++;
    }

    if (msa_labels.count(tip.second) == 0) {
      LOG_ERROR << "ERROR: Taxon name not found in the alignment: "
                << tip.second << endl;
      missing_taxa++;
    }
  }
  ...
  if (missing_taxa > 0)
    throw runtime_error("Please check that sequence labels in the alignment "
                        "and in the tree file are identical!");
...
/* This function is called after MPI init, but before thread init, MSA loading
 * etc. */
void check_options_early(Options &opts) {
  ...
  if (!opts.constraint_tree_file.empty() &&
    ...
    throw runtime_error(" User and parsimony starting trees are not supported "
                        "in combination with "
  ...
  if (opts.coarse() && opts.num_ranks > opts.num_workers) {
    throw runtime_error("Unsupported parallelization topology!\n"
                        "NOTE:  Multiple MPI ranks per worker are not allowed "
                        "in coarse-grained mode.\n");
  }

  if (opts.coarse() &&
      (opts.num_ranks * opts.num_threads % opts.num_workers != 0)) {
  ...
  /* writing interim result files is not supported in coarse+MPI mode -> too
   * much hassle */
  ...
  if (opts.command == Command::ancestral) {
    if (opts.use_pattern_compression)
      throw runtime_error("Pattern compression is not supported in ancestral "
                          "state reconstruction mode!");
    if (opts.use_repeats)
      throw runtime_error("Site repeats are not supported in ancestral state "
                          "reconstruction mode!");
    if (opts.use_rate_scalers)
      throw runtime_error("Per-rate scalers are not supported in ancestral "
                          "state reconstruction mode!");
    if (opts.num_ranks > 1)
      throw runtime_error("MPI parallelization is not supported in ancestral "
                          "state reconstruction mode!");
  }

  if (opts.command == Command::sitelh) {
    if (opts.num_ranks > 1)
      throw runtime_error("MPI parallelization is not supported in per-site "
                          "likelihood computation mode!");
    if (!opts.weights_file.empty())
      throw runtime_error("Custom site weights are not supported in per-site "
                          "likelihood computation mode!");
  }

  /* autodetect if we can use partial RBA loading */
  opts.use_rba_partload &=
      (opts.num_ranks > 1 &&
       !opts.coarse()); // only useful for fine-grain MPI runs
  opts.use_rba_partload &= (!opts.start_trees.count(
      StartingTree::parsimony)); // does not work with parsimony
  opts.use_rba_partload &=
      (opts.command ==
           Command::search || // currently doesn't work with bootstrap
       opts.command == Command::evaluate ||
       opts.command == Command::ancestral);

  LOG_DEBUG << "RBA partial loading: " << (opts.use_rba_partload ? "ON" : "OFF")
            << endl;
}
...
void check_options(RaxmlInstance &instance) {
  ...
  if (opts.safety_checks.isset(SafetyCheck::perf_threads)) {
    if (ParallelContext::master_rank() && ParallelContext::num_procs() > 1) {
      ...
      if (ParallelContext::threads_per_group() > res.num_threads_response) {
        LOG_WARN << endl;
        LOG_WARN << "WARNING: You might be using too many threads ("
                 << ParallelContext::num_procs() << ") for your alignment with "
                 << (opts.use_pattern_compression
                         ? to_string(instance.parted_msa->total_patterns()) +
                               " unique patterns."
                         : to_string(instance.parted_msa->total_sites()) +
                               " alignment sites.")
                 << endl;
        LOG_WARN << "NOTE:    For the optimal throughput, please consider "
                    "using fewer threads "
                 << endl;
        LOG_WARN << "NOTE:    and parallelize across starting trees/bootstrap "
                    "replicates."
                 << endl;
        LOG_WARN << "NOTE:    As a general rule-of-thumb, please assign at "
                    "least 200-1000 "
                    "alignment patterns per thread."
                 << endl
                 << endl;

        if (ParallelContext::threads_per_group() >
            2 * res.num_threads_response) {
          throw runtime_error(
              "Too few patterns per thread! "
              "RAxML-NG will terminate now to avoid wasting resources.\n"
              "NOTE:  Please reduce the number of threads (see guidelines "
              "above).\n"
              "NOTE:  This check can be disabled with the '--force "
              "perf_threads' option.");
        }
      }
    }
  ...
  if (opts.safety_checks.isset(SafetyCheck::model_rate_scalers)) {
    if (instance.parted_msa->taxon_count() > RAXML_RATESCALERS_TAXA &&
      ...
      LOG_INFO << "\nNOTE: Per-rate scalers were automatically enabled to "
  ...
  /* make sure we do not check for convergence too often in coarse-grained
   * parallelization mode */
  instance.opts.bootstop_interval =
      std::max(opts.bootstop_interval, opts.num_workers * 2);
}
...
void check_oversubscribe(RaxmlInstance &instance) {
  ...
  if (opts.safety_checks.isset(SafetyCheck::perf_threads)) {
    ...
    if (ParallelContext::master()) {
      double sync_time =
          1000. * (global_timer().elapsed_seconds() - start) / iters;
      ...
      if (sync_time > 5. + 0.1 * log2(ParallelContext::num_nodes())) {
        throw runtime_error(
            ...
            "Parallelization#core-oversubscription\n"
...
void autotune_threads(RaxmlInstance &instance) {
  ...
  StaticResourceEstimator resEstimator(*instance.parted_msa, instance.opts);
  auto res = resEstimator.estimate();
  unsigned int est_threads_throughput =
      (unsigned int)res.num_threads_throughput;
  ...
  unsigned int max_workers_mem =
      0.9 * num_ranks * sysutil_get_memtotal() / res.total_mem_size;
  ...
  if (opts.num_workers == 0) {
    if (max_workers > 1) {
      ...
      /* workers spanning multiple MPI ranks are not supported atm -> check for
       * this */
  ...
  if (opts.num_threads == 0) {
    auto opt_rank_threads =
        std::max(est_threads_response * opts.num_workers / num_ranks, 1u);
    opts.num_threads =
        opt_rank_threads <= opts.num_threads_max
            ? opt_rank_threads
  ...
  auto max_threads_per_worker =
      (unsigned int)instance.parted_msa->total_patterns();
  if (threads_per_worker > max_threads_per_worker) {
    threads_per_worker = std::max(max_threads_per_worker, 1u);
    opts.num_threads = threads_per_worker * opts.num_workers / num_ranks;

    LOG_WARN << "WARNING: Requested more threads per worker than available "
                "alignment patterns ("
             << max_threads_per_worker << ")." << endl
             << "Reducing thread count to avoid crash." << endl
             << endl;
  }

  LOG_INFO << "Parallelization scheme autoconfig: " << opts.num_workers
           << " worker(s) x " << threads_per_worker << " thread(s)" << endl
           << endl;
}
...
void load_msa_weights(MSA &msa, const Options &opts) {
  ...
  if (!opts.weights_file.empty()) {
    ...
    /* RBA file contains collapsed site weights -> not compatible with weights
     * file! */
    assert(opts.msa_format != FileFormat::binary);

    FILE *f = fopen(opts.weights_file.c_str(), "r");
    if (!f)
      throw runtime_error("Unable to open site weights file: " +
                          opts.weights_file);
    ...
    while ((fres = fscanf(f, "%jd", &x)) != EOF) {
      if (!fres) {
        ...
        buf[c] = 0;
        fclose(f);
        throw runtime_error("Invalid site weight entry found near: " +
                            string(buf));
      } else if (x <= 0) {
        fclose(f);
    ...
    if (w.size() != msa.length()) {
      throw runtime_error(
          "Site weights file contains the wrong number of entries: " +
          to_string(w.size()) + " (expected: " + to_string(msa.length()) + ")" +
          "\nPlease check that this file contains one positive integer per "
          "site: " +
          opts.weights_file);
    }
    msa.weights(w);
...
void load_msa(RaxmlInstance &instance) {
  ...
  if (msa.probabilistic() && opts.use_prob_msa) {
    ...
    if (parted_msa.part_count() > 1)
      throw runtime_error("Partitioned probabilistic alignments are not "
                          "supported yet, sorry...");
  } else
    instance.opts.use_prob_msa = false;
  ...
  if (opts.use_pattern_compression) {
    ...
    // temp workaround: since MSA pattern compression calls rand(), it will
    // change all random numbers generated afterwards. so just reset seed to the
    // initial value to ensure that starting trees, BS replicates etc. are the
    // same regardless whether pat.comp is ON or OFF
  ...
  LOG_INFO << "Alignment comprises " << parted_msa.part_count()
           << " partitions and " << parted_msa.total_length()
           << (opts.use_pattern_compression ? " patterns" : " sites") << endl
           << endl;
  ...
  if (ParallelContext::master_rank() && !instance.opts.use_prob_msa &&
      !instance.opts.binary_msa_file().empty()) {
    ...
    if (sysutil_file_exists(binary_msa_fname) && !opts.redo_mode &&
      ...
      LOG_INFO << "NOTE: Binary MSA file already exists: " << binary_msa_fname
               << endl
               << endl;
...
Tree generate_tree(const RaxmlInstance &instance, StartingTree type,
  ...
  case StartingTree::user: {
    ...
    *instance.start_tree_stream >> tree;
    ...
    LOG_DEBUG << "Loaded user starting tree with " << tree.num_tips()
              << " taxa from: " << opts.tree_file << endl;
    ...
    if (!instance.constraint_tree.empty()) {
      ...
      if (!instance.constraint_tree.compatible(tree)) {
        throw runtime_error("User starting tree incompatible with the "
                            "specified topological constraint!");
      }
    }
  ...
  case StartingTree::random:
    ...
    LOG_VERB_TS << "Generating a RANDOM starting tree, seed: " << random_seed
                << endl;
  ...
  case StartingTree::parsimony: {
    ...
    tree = Tree::buildParsimonyConstrained(pars_msa, random_seed, &score,
                                           instance.constraint_tree,
                                           instance.tip_msa_idmap);

    LOG_WORKER_TS(LogLevel::verbose)
        << "Generated a PARSIMONY starting tree, seed: " << random_seed
        << ", score: " << score << endl;
...
void load_start_trees(RaxmlInstance &instance) {
  ...
  if (instance.opts.start_trees.count(StartingTree::user) > 0) {
    ...
    // until we read trees from the file. that's why we update num_searches
    // here.
...
void load_checkpoint(RaxmlInstance &instance, CheckpointManager &cm) {
  ...
  if (!instance.opts.redo_mode) {
    if (cm.read()) {
      LOG_DEBUG << "NOTE: Loaded checkpoint file: "
                << instance.opts.checkp_file() << endl;
    }
    ...
    // NB: consider BS trees from the previous run when performing bootstopping
    // test
    ...
    /* add in-progress tree ids to the "done" list, to exclude them from coarse
     * load balancing */
    if (ParallelContext::master()) {
    ...
    if (!instance.done_ml_trees.empty() || !instance.done_bs_trees.empty()) {
      LOG_INFO_TS << "NOTE: Resuming execution from checkpoint "
                  ...
                  << ", bootstraps: " << ckpfile.bs_trees.size() << ")" << endl;
    }

    //    printf("DONE: %lu %lu\n", ParallelContext::rank_id(),
    //    instance.done_ml_trees.size());
  }
}
...
void load_constraint(RaxmlInstance &instance) {
  ...
  if (!instance.opts.constraint_tree_file.empty()) {
    if (!sysutil_file_exists(opts.constraint_tree_file))
      throw runtime_error("Constraint tree file not found: " +
                          opts.constraint_tree_file);

    NewickStream nw_cons(instance.opts.constraint_tree_file, std::ios::in);
    Tree &cons_tree = instance.constraint_tree;

    LOG_INFO_TS << "Loading constraint tree from: " << opts.constraint_tree_file
                << endl;
    nw_cons >> cons_tree;

    LOG_INFO_TS << "Loaded "
                << (cons_tree.num_tips() == parted_msa.taxon_count() ? ""
                                                                     : "non-")
                << "comprehensive constraint tree with " << cons_tree.num_tips()
                << " taxa" << endl
                << endl;
    ...
    // check if taxa names are consistent between constraint tree and MSA
    {
      ...
      if (!missing_taxa.empty()) {
        ...
        ss << "Following " << missing_taxa.size()
           << " taxa present in the constraint tree can not be found in the "
              "alignment: "
           << endl;
    ...
    //    /* branch lengths in constraint tree can be misleading, so just reset
    //    them to default value */ cons_tree.reset_brlens();

    if (cons_tree.num_tips() < parted_msa.taxon_count()) {
      // incomplete constraint tree -> adjust tip IDs such that all taxa in the
      // constraint tree go before the remaining free taxa
      ...
      throw runtime_error("You provided a comprehensive, fully-resolved tree "
                          "as a topological constraint.\n"
                          "Since this is almost certainly not what you "
                          "intended, RAxML-NG will now exit...");
    }

    /* make sure tip indices are consistent between MSA and pll_tree */
    cons_tree.reset_tip_ids(instance.tip_id_map);

    //    pll_utree_show_ascii(&cons_tree.pll_utree_root(), PLL_UTREE_SHOW_LABEL
...
void build_parsimony_msa(RaxmlInstance &instance) {
  ...
  instance.parted_msa_parsimony.reset(
      new ParsimonyMSA(instance.parted_msa, attrs));
}
...
void thread_start_trees(RaxmlInstance &instance, StartingTree st_tree_type,
  ...
  for (size_t i = 0; i < seeds.size(); ++i) {
    if (i % ParallelContext::num_threads() == ParallelContext::thread_id())
      instance.start_trees[offset + i] =
          generate_tree(instance, st_tree_type, seeds[i]);
  }
}
...
void build_start_trees(RaxmlInstance &instance, unsigned int num_threads = 1) {
  ...
  for (auto &st_tree : opts.start_trees) {
    ...
    switch (st_tree_type) {
    case StartingTree::user:
      LOG_INFO_TS << "Loading user starting tree(s) from: " << opts.tree_file
                  << endl;
      if (!sysutil_file_exists(opts.tree_file))
        throw runtime_error("File not found: " + opts.tree_file);
      instance.start_tree_stream.reset(
          new NewickStream(opts.tree_file, std::ios::in));
      break;
    case StartingTree::random:
      LOG_INFO_TS << "Generating " << st_tree_count
                  << " random starting tree(s) with "
                  << parted_msa.taxon_count() << " taxa" << endl;
      break;
    case StartingTree::parsimony:
      build_parsimony_msa(instance);
      LOG_INFO_TS << "Generating " << st_tree_count
                  << " parsimony starting tree(s) with "
                  << parted_msa.taxon_count() << " taxa" << endl;
      break;
    default:
      assert(0);
    ...
    if (num_threads != 1 && st_tree_type == StartingTree::parsimony) {
      ...
      auto thread_fn = std::bind(thread_start_trees, std::ref(instance),
                                 st_tree_type, std::cref(seeds), old_size);

      assert(num_threads > 0);
      auto mem_per_thread = instance.parted_msa_parsimony->memsize_estimate();
      LOG_VERB << "Estimated memory per parsimony thread: "
               << mem_per_thread / 1024 / 1024 << " MB" << endl;
      unsigned int num_threads_max =
          0.7 * sysutil_get_memtotal() / mem_per_thread;
      num_threads = std::min(num_threads, num_threads_max);
      LOG_INFO << "Parallel parsimony with " << num_threads << " threads"
               << endl;
      ParallelContext::init_pthreads_custom(opts, thread_fn, num_threads,
                                            num_threads);
      thread_fn();
...
void balance_load(RaxmlInstance &instance) {
  ...
  for (auto const &pinfo : instance.parted_msa->part_list()) {
    part_sizes.assign_sites(i, 0, pinfo.length(),
                            pinfo.model().clv_entry_size());
    ++i;
  }

  instance.proc_part_assign = instance.load_balancer->get_all_assignments(
      part_sizes, ParallelContext::threads_per_group());

  LOG_INFO_TS << "Data distribution: "
              << PartitionAssignmentStats(instance.proc_part_assign) << endl;
  LOG_VERB << endl << instance.proc_part_assign;
}

PartitionAssignmentList balance_load(RaxmlInstance &instance,
                                     WeightVectorList part_site_weights) {
  /* This function is used to re-distribute sites across processes for each
   * bootstrap replicate. Since during bootstrapping alignment sites are sampled
   * with replacement, some sites will be absent from BS alignment. Therefore,
   * site distribution computed for original alignment can be suboptimal for BS
   * replicates. Here, we recompute the site distribution, ignoring all sites
   * that are not present in BS replicate (i.e., have weight of 0 in
   * part_site_weights).
   * */
  ...
  assign_list = instance.load_balancer->get_all_assignments(
      part_sizes, ParallelContext::threads_per_group());

  LOG_VERB_TS << "Data distribution: " << PartitionAssignmentStats(assign_list)
              << endl;
  ...
  //  LOG_VERB_TS << "(uncompressed) Data distribution: " <<
  //  PartitionAssignmentStats(instance.proc_part_assign) << endl; LOG_DEBUG <<
  //  endl << instance.proc_part_assign;

  return assign_list;
}

void balance_load_coarse(RaxmlInstance &instance,
                         const CheckpointFile &ckpfile) {
  ...
  CoarseAssignmentList start_tree_assign =
      instance.coarse_load_balancer->get_all_assignments(todo_start_trees,
                                                         num_workers);
  CoarseAssignmentList bs_tree_assign =
      instance.coarse_load_balancer->get_all_assignments(todo_bs_trees,
                                                         num_workers);
...
void generate_bootstraps(RaxmlInstance &instance,
                         const CheckpointFile &checkp) {
  if (instance.opts.command == Command::bootstrap ||
      instance.opts.command == Command::all ||
    ...
    /* in parallel parsimony mode, starting trees & replicate MSAs will be
     * generated later "just-in-time" */
    if (instance.opts.use_par_pars)
      return;

    /* generate starting trees for bootstrap searches */
    auto start_tree_type = instance.opts.use_bs_pars ? StartingTree::parsimony
                                                     : StartingTree::random;
    ...
    for (size_t b = 0; b < instance.opts.num_bootstraps; ++b) {
      //      /* check if this BS was already computed in the previous run and
      //      saved in checkpoint */ if (b < checkp.bs_trees.size())
      //        continue;

      instance.bs_reps.emplace_back(
          bg.generate(*instance.parted_msa, seeds[b]));
    }
  }
  RAXML_UNUSED(checkp); // might need it again for re-using previously computed
                        // replicates
}
...
void init_ancestral(RaxmlInstance &instance) {
  if (instance.opts.command == Command::ancestral) {
    ...
    instance.ancestral_states =
        make_shared<AncestralStates>(tree.num_inner(), parted_msa);
  }
}
...
void reroot_tree_with_outgroup(const Options &opts, Tree &tree,
                               bool add_root_node) {
  if (!opts.outgroup_taxa.empty()) {
    try {
...
void draw_bootstrap_support(RaxmlInstance &instance, Tree &ref_tree,
  ...
  for (auto metric : instance.opts.bs_metrics) {
    ...
    if (metric == BranchSupportMetric::fbp) {
      ...
      sup_tree =
          make_shared<TransferBootstrapTree>(ref_tree, instance.opts.tbe_naive);
...
bool check_bootstop(const RaxmlInstance &instance,
                    const TreeTopologyList &bs_trees, bool print = false) {
  if (!instance.bootstop_checker)
    return false;
  ...
  if (print) {
    ...
    LOG_INFO << " # trees       "
             << " avg WRF      "
             << " avg WRF in %      "
             << " # perms: wrf <= " << setprecision(2)
             << opts.bootstop_cutoff * 100 << " %    "
             << " converged?  " << endl;
  }
  ...
  for (auto &it : bs_trees) {
    ...
    if (bs_num % opts.bootstop_interval == 0 || bs_num == bs_trees.size()) {
      ...
      if (print) {
        LOG_INFO << setw(8) << bs_num << " " << setw(14) << setprecision(3)
                 << bootstop_checker->avg_wrf() << "   " << setw(16)
                 << setprecision(3) << bootstop_checker->avg_pct() << "   "
  ...
  if (print) {
    LOG_INFO << "Bootstopping test "
             << (converged ? "converged" : "did not converge") << " after "
             << bootstop_checker->num_bs_trees() << " trees" << endl
             << endl;
  }
...
TreeTopologyList read_newick_trees(Tree &ref_tree, const std::string &fname,
  ...
  while (boots.peek() != EOF) {
    ...
    if (tree.num_tips() != ref_tree.num_tips()) {
      throw runtime_error(
          tree_kind_cap + " tree #" + to_string(bs_num + 1) +
  ...
  LOG_INFO << "Loaded " << trees.size() << " trees with " << ref_tree.num_tips()
           << " taxa." << endl
           << endl;
...
TreeTopologyList read_bootstrap_trees(const RaxmlInstance &instance,
                                      Tree &ref_tree) {
  auto bs_trees = read_newick_trees(
      ref_tree, instance.opts.bootstrap_trees_file(), "bootstrap");

  if (bs_trees.size() < 2) {
    throw runtime_error(
        "You must provide a file with multiple bootstrap trees!");
  }
...
void command_support(RaxmlInstance &instance) {
  ...
  if (!ref_tree.binary())
    throw runtime_error(
        "Reference tree contains multifurcations, this is not supported!");
...
void command_rfdist(RaxmlInstance &instance) {
  ...
  if (instance.start_trees.size() < 2)
    throw runtime_error("Cannot compute RF distances since tree file contains "
                        "fewer than 2 trees!");
...
void command_consense(RaxmlInstance &instance) {
  ...
  if (instance.start_trees.size() < 2)
    throw runtime_error(
        "Cannot consensus tree since tree file contains fewer than 2 trees!");

  instance.consens_tree.reset(
      new ConsensusTree(instance.start_trees, opts.consense_cutoff));
    ...
    try {
      ...
      if (terrace_size > 1) {
        ...
        if (!instance.opts.terrace_file().empty()) {
          ...
          LOG_INFO
              << "Tree terrace (in compressed Newick format) was saved to: "
          ...
          if (terrace_size <= instance.opts.terrace_maxsize) {
            ...
            LOG_INFO
                << "Tree terrace (in multi-line Newick format) was saved to: "
...
void print_ic_scores(const RaxmlInstance &instance, double loglh) {
  ...
  LOG_INFO << "Free parameters (model + branch lengths): " << free_params
           << endl
           << endl;
  ...
  if (free_params >= sample_size) {
    LOG_WARN << "WARNING: Number of free parameters (K=" << free_params << ") "
             ...
             << "         This might lead to overfitting and compromise tree "
                "inference results!\n"
             << endl
             << endl;
  }
...
void print_final_output(const RaxmlInstance &instance,
                        const CheckpointFile &checkp) {
  ...
  if (opts.command == Command::search || opts.command == Command::all ||
      ...
      opts.command == Command::ancestral) {
    auto model_log_lvl =
        parted_msa.part_count() > 1 ? LogLevel::verbose : LogLevel::info;
  ...
  if (opts.command == Command::search || opts.command == Command::all ||
    ...
    if (!collapsed_tree.binary()) {
      ...
      auto collapsed_count =
          best_tree.num_branches() - collapsed_tree.num_branches();
      LOG_WARN << "WARNING: Best ML tree contains " << collapsed_count
               << " near-zero branches!" << endl
               << endl;
    ...
    if (!opts.best_tree_file().empty()) {
      ...
      LOG_INFO << "Best ML tree saved to: "
               << sysutil_realpath(opts.best_tree_file()) << endl;
    }

    if (opts.brlen_linkage == PLLMOD_COMMON_BRLEN_UNLINKED &&
        !opts.partition_trees_file().empty()) {
    ...
    if (checkp.ml_trees.size() > 1 && !opts.ml_trees_file().empty()) {
      ...
      LOG_INFO << "All ML trees saved to: "
               << sysutil_realpath(opts.ml_trees_file()) << endl;
    }
  }
  ...
  if (opts.command == Command::all || opts.command == Command::support) {
    ...
    for (const auto &it : instance.support_trees) {
      ...
      if (!sup_file.empty()) {
        ...
        LOG_INFO << "Best ML tree with " << metric_name
                 << " support values saved to: " << sysutil_realpath(sup_file)
                 << endl;
      }
    }
  }
  ...
  if (opts.command == Command::consense) {
    ...
    if (!cons_file.empty()) {
      ...
      LOG_INFO << opts.consense_type_name()
               << " consensus tree saved to: " << sysutil_realpath(cons_file)
               << endl;
    }
  }
  ...
  if (opts.command == Command::search || opts.command == Command::all ||
    ...
    if (!opts.best_model_file().empty()) {
      ...
      model_stream << fixed
                   << setprecision(logger().precision(LogElement::model));
      model_stream << parted_msa;

      LOG_INFO << "Optimized model saved to: "
               << sysutil_realpath(opts.best_model_file()) << endl;
    }
  }

  if (opts.command == Command::bootstrap || opts.command == Command::all) {
    // TODO now only master process writes the output, this will have to change
    // with coarse-grained parallelization scheme (parallel start
    // trees/bootstraps)
    if (!opts.bootstrap_trees_file().empty()) {
      ...
      LOG_INFO << "Bootstrap trees saved to: "
               << sysutil_realpath(opts.bootstrap_trees_file()) << endl;
    }
  }
  ...
  if (opts.command == Command::bsmsa || opts.write_bs_msa) {
    if (!opts.bootstrap_msa_file(1).empty()) {
      ...
      auto max_bs_trees =
          opts.write_bs_msa ? checkp.bs_trees.size() : opts.num_bootstraps;
      ...
      LOG_INFO << "Bootstrap replicate MSAs saved to: "
               ...
               << endl
               << "                                   "
               << sysutil_realpath(opts.bootstrap_msa_file(max_bs_trees))
               << endl;
      ...
      if (print_part_file) {
        ...
        LOG_INFO << endl;
        LOG_INFO
            << "Partition file for (all) bootstrap replicate MSAs saved to: "
            << sysutil_realpath(opts.bootstrap_partition_file()) << endl
            << endl;

        LOG_INFO << "IMPORTANT: You MUST use the aforementioned adjusted "
                    "partitioned file"
                 << endl
                 << "           when running tree searches on bootstrap "
                    "replicate MSAs!"
                 << endl;
      }
    }
  ...
  if (opts.command == Command::rfdist) {
    ...
    LOG_RESULT << "Average absolute RF distance in this tree set: "
               << rfcalc.avg_rf() << endl;
    LOG_RESULT << "Average relative RF distance in this tree set: "
               << rfcalc.avg_rrf() << endl;
    LOG_RESULT << "Number of unique topologies in this tree set: "
               << rfcalc.num_uniq_trees() << endl;

    if (!opts.rfdist_file().empty()) {
      fstream fs(opts.rfdist_file(), ios::out);
      fs << rfcalc;

      LOG_INFO << "\nPairwise RF distances saved to: "
               << sysutil_realpath(opts.rfdist_file()) << endl;
    }
  }
  ...
  if (opts.command == Command::ancestral) {
    ...
    if (!opts.asr_probs_file().empty()) {
      ...
      LOG_INFO << "Marginal ancestral probabilities saved to: "
               << sysutil_realpath(opts.asr_probs_file()) << endl;
    }
    ...
    if (!opts.asr_states_file().empty()) {
      ...
      LOG_INFO << "Reconstructed ancestral sequences saved to: "
               << sysutil_realpath(opts.asr_states_file()) << endl;
    }
    ...
    if (!opts.asr_tree_file().empty()) {
      ...
      LOG_INFO << "Node-labeled tree saved to: "
               << sysutil_realpath(opts.asr_tree_file()) << endl;
    }
  }
  ...
  if (opts.command == Command::sitelh) {
    if (!opts.sitelh_file().empty()) {
      ...
      fs << instance.persite_loglh.size() << " " << parted_msa.total_sites()
         << endl;
      ...
      for (auto &tree_slh : instance.persite_loglh) {
        ...
        for (const auto &coord : parted_msa.full_to_parted_sitemap()) {
          /* coord.first = partition, coord.second = site (pattern) within
           * partition */
      ...
      LOG_INFO << "Per-site LogLikelihoods saved to: "
               << sysutil_realpath(opts.sitelh_file()) << endl;
    }
  }

  if (!opts.log_file().empty())
    LOG_INFO << "\nExecution log saved to: "
             << sysutil_realpath(opts.log_file()) << endl;

  LOG_INFO << "\nAnalysis started: " << global_timer().start_time();
  LOG_INFO << " / finished: " << global_timer().current_time() << std::endl;
  LOG_INFO << "\nElapsed time: " << FMT_PREC3(global_timer().elapsed_seconds())
           << " seconds";
  if (checkp.elapsed_seconds > 0.) {
    LOG_INFO << " (this run) / ";
    LOG_INFO << FMT_PREC3(checkp.elapsed_seconds +
                          global_timer().elapsed_seconds())
...
void print_resources(const RaxmlInstance &instance) {
  ...
  LOG_INFO << "* Estimated memory requirements                : "
           << (size_t)(((float)res.total_mem_size) / (1024 * 1024) + 1) << " MB"
           << endl
           << endl;
  ...
  LOG_INFO << endl
           ...
           << endl
           << "Actual memory consumption and parallel performance on your "
              "system may differ!"
           << endl
           << endl;
}
...
void init_parallel_buffers(const RaxmlInstance &instance) {
  ...
  // we need 2 doubles for each partition AND threads to perform parallel
  // reduction, so resize the buffer accordingly
  const size_t reduce_buffer_size =
      std::max<size_t>(1024u, 2 * sizeof(double) * parted_msa.part_count() *
  ...
  if (ParallelContext::num_ranks() > 1) {
    ...
    auto tree_size =
        BinaryStream::serialized_size(instance.random_tree.topology());

    // buffer needs enough space to store serialized model parameters
    worker_buf_size = model_size;

    // for coarse-grained, add extra space to store ML/BS trees sent from
    // workers to master
    if (ParallelContext::num_groups() > 1) {
      // add 1 extra tree for load imbalance
      auto max_trees_per_worker =
          (opts.bootstop_interval / ParallelContext::num_groups() + 1) + 1;
      auto max_trees_per_rank =
          max_trees_per_worker * ParallelContext::num_local_groups();
  ...
  LOG_INFO << "Parallel reduction/worker buffer size: "
           << reduce_buffer_size / 1024 << " KB  / " << worker_buf_size / 1024
           << " KB\n\n";
...
void thread_infer_ml(RaxmlInstance &instance, CheckpointManager &cm) {
  ...
  auto const &part_assign =
      instance.proc_part_assign.at(ParallelContext::local_proc_id());
  ...
  unsigned int batch_id =
      (instance.done_ml_trees.size() / opts.bootstop_interval) + 1;

  auto ckp_tree_index =
      instance.run_phase == RaxmlRunPhase::mlsearch ? checkp.tree_index : 0;
  ...
  for (auto start_tree_num : worker.start_trees) {
    ...
    if (ckp_tree_index == start_tree_num) {
      ...
      treeinfo.reset(new TreeInfo(opts, tree, master_msa,
                                  instance.tip_msa_idmap, part_assign));
    }
    ...
    auto log_level =
        instance.start_trees.size() > 1 ? LogLevel::result : LogLevel::info;
    ...
    if (opts.command == Command::evaluate || opts.command == Command::sitelh ||
      ...
      if (opts.optimize_brlen || opts.optimize_model) {
        LOG_INFO_TS << "Tree #" << start_tree_num
                    << ", initial LogLikelihood: " << FMT_LH(treeinfo->loglh())
                    << endl;
        LOG_PROGR << endl;
...
void thread_infer_bootstrap(RaxmlInstance &instance, CheckpointManager &cm) {
  ...
  auto gather_bs_trees = [&instance, &opts,
                          &cm](unsigned int &batch_start,
                               unsigned int &batch_end) -> void {
    ParallelContext::global_thread_barrier();
    ...
    if (instance.bootstop_checker) {
      if (ParallelContext::master()) {
        ...
        instance.bs_converged =
            instance.bootstop_checker->converged(opts.random_seed);

        if (instance.bs_converged) {
          auto num_bs_trees = cm.checkp_file().bs_trees.size();
          LOG_INFO_TS << "Bootstrapping converged after " << num_bs_trees
                      << " replicates." << endl;
        }
      }
    ...
    batch_end =
        std::min(opts.num_bootstraps, batch_end + opts.bootstop_interval);
  };
  ...
  ParallelContext::global_master_broadcast(&bs_batch_start,
                                           sizeof(unsigned int));
  ...
  if (bs_batch_start == 0) {
    if (opts.command == Command::all) {
      ...
      LOG_INFO_TS << "ML tree search completed, best tree logLH: "
                  << FMT_LH(cm.checkp_file().ml_trees.best_score()) << endl
                  << endl;
    }

    LOG_INFO_TS << "Starting bootstrapping analysis with "
                << opts.num_bootstraps << " replicates." << endl
                << endl;
  } else {
    LOG_INFO_TS << "Continuing bootstrapping analysis with "
                << opts.num_bootstraps << " replicates." << endl
                << endl;
  }

  auto bs_batch_offset = bs_batch_start % opts.bootstop_interval;
  unsigned int bs_batch_end =
      std::min(bs_batch_start - bs_batch_offset + opts.bootstop_interval,
               opts.num_bootstraps);
  auto bs_num = worker.bs_trees.cbegin();

  auto ckp_tree_index =
      instance.run_phase == RaxmlRunPhase::bootstrap ? checkp.tree_index : 0;
  ...
  while (!instance.bs_converged && bs_num != worker.bs_trees.cend()) {
    if (instance.opts.use_par_pars) {
      if (ParallelContext::group_master_thread()) {
        ...
        worker.cur_bs_start_tree =
            generate_tree(instance, start_tree_type, bs_seed);
    ...
    // rebalance sites
    if (ParallelContext::group_master_thread()) {
      worker.proc_part_assign =
          balance_load(instance, worker.cur_bs_rep.site_weights);
    }
    ParallelContext::thread_barrier();

    auto const &bs_part_assign =
        worker.proc_part_assign.at(ParallelContext::local_proc_id());

    if (ckp_tree_index == *bs_num) {
      // restore search state from checkpoint (tree + model params)
      treeinfo.reset(new TreeInfo(opts, checkp.tree, master_msa,
                                  instance.tip_msa_idmap, bs_part_assign,
                                  worker.cur_bs_rep.site_weights));
      assign_models(*treeinfo, checkp);
    } else {
      if (ParallelContext::group_master_thread())
        checkp.tree_index = *bs_num;
      treeinfo.reset(new TreeInfo(opts, worker.cur_bs_start_tree, master_msa,
                                  instance.tip_msa_idmap, bs_part_assign,
                                  worker.cur_bs_rep.site_weights));
    }
  ...
  /* special case: if this worker had no bsreps in last batch, it still must
   * synchronize! */
...
void master_main(RaxmlInstance &instance, CheckpointManager &cm) {
  ...
  if (!opts.redo_mode && sysutil_file_exists(opts.checkp_file()) &&
  ...
  if (!instance.opts.redo_mode &&
      sysutil_file_exists(instance.opts.start_tree_file()) &&
      instance.opts.num_searches > 0 &&
      !(instance.opts.constraint_tree_file.empty() &&
        instance.opts.use_old_constraint)) {
  ...
  if (instance.start_trees.size() < opts.num_searches) {
    if (ParallelContext::master_rank() || opts.start_tree_file().empty() ||
      ...
      /* only master MPI rank generates starting trees (doesn't work with old
       * constrained search) */
  ...
  ParallelContext::init_pthreads(
      opts, std::bind(thread_main, std::ref(instance), std::ref(cm)));
  ...
  if (opts.msa_format == FileFormat::binary && opts.use_rba_partload) {
    ...
    for (size_t i = 0; i < opts.num_threads; ++i) {
      auto thread_ranges =
          instance.proc_part_assign.at(ParallelContext::local_proc_id() + i);
    ...
    RBAStream bs(opts.msa_file);
    bs >> RBAStream::RBAOutput(parted_msa, RBAStream::RBAElement::seqdata,
                               &local_part_ranges);
  }

  // TEMP WORKAROUND: here we reset random seed once again to make sure that BS
  // replicates are not affected by the number of ML search starting trees that
...
int internal_main(int argc, char **argv, void *comm) {
  ...
  case Command::ancestral:
    if (!opts.redo_mode && opts.result_files_exist()) {
      LOG_ERROR
          << endl
          << "ERROR: Result files for the run with prefix `"
  ...
  try {
    ...
    if (ParallelContext::master() && !instance.opts.log_file().empty()) {
      auto mode = !instance.opts.redo_mode &&
    ...
    if (opts.redo_mode) {
      LOG_WARN
          << "WARNING: Running in REDO mode: existing checkpoints are ignored, "
    ...
    if (opts.force_mode) {
      LOG_WARN << "WARNING: Running in FORCE mode: "
               ...
               << " safety checks are disabled!" << endl
               << endl;
    }
    ...
    case BootstopCriterion::autoMRE:
      instance.bootstop_checker.reset(
          new BootstopCheckMRE(opts.num_bootstraps, opts.bootstop_cutoff,
    ...
    default:
      throw runtime_error(
          "Only autoMRE bootstopping criterion is supported for now, sorry!");
    }
    ...
    case Command::parse: {
      if (ParallelContext::master_rank()) {
        ...
        if (!opts.tree_file.empty() && !instance.constraint_tree.empty()) {
          LOG_INFO << "All starting trees (" << opts.num_searches
                   << ") are compatible with the topological constraint."
                   << endl
                   << endl;
        }
      }
      ...
      if (ParallelContext::num_nodes() > 1)
        LOG_WARN << "WARNING: Running --parse on multiple nodes is wasting "
                    "resources!"
                 << endl
                 << endl;
    ...
    case Command::start: {
      auto num_threads =
          opts.num_threads ? opts.num_threads : opts.num_threads_max;
      load_parted_msa(instance);
      load_constraint(instance);
      build_start_trees(instance, num_threads);
      if (!opts.start_tree_file().empty()) {
        LOG_INFO << "\nAll starting trees saved to: "
                 << sysutil_realpath(opts.start_tree_file()) << endl
                 << endl;
      } else {
